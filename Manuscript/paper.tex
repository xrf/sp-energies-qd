...
\begin{align*}
  v(\bm{r}, \bm{r}') \equiv \frac{1}{\left|\bm{r} - \bm{r}'\right|}
\end{align*}
...
\end{align}
The operator $\hat{\bm r}_j$ is the position operator of the $j$-th particle,\footnote{The choice and ordering of the particle labels $j$ is unimportant; they only exist for the purposes of bookkeeping.} and $\hat{\bm p}_j \equiv -\mathrm{i} \hat{\bm{\nabla}}_{\bm{r}_j}$ is its momentum operator.

The first step towards a solution is to solve \emph{just} the noninteracting part of the many-body Hamiltonian $\hat{H}^{[1]}$, which reduces to $N$ independent single-particle Hamiltonians $\hat{h}$ each of the form:
\begin{align*}
  \hat{h} = \frac{1}{2} \hat{\bm{\nabla}}^2 + \frac{1}{2} \omega^2 \hat{\bm{r}}^2
\end{align*}
This is a 2-dimensional harmonic oscillator problem for a single particle, of which analytic solutions are known.  There are various equivalent ways to construct the solutions.  For our purposes, we shall use the Fock-Darwin states, which can be conveniently described polar coordinates $(\rho, \varphi)$ and have the elegant property of conserving the orbital angular momentum $\hat{L}_{\mathrm{z}} \equiv -\mathrm{i} \partial / (\partial \varphi)$.  The wave functions can be factorized into radial and angular components,\citep[Appx.\ A]{lohne2010coupled}
\begin{align}
  \phi^{\text{FD}}_{n, m_\ell}(\rho, \varphi) &\equiv \sqrt\omega \phi^{\text{FDr}}_{n, |m_\ell|}(\sqrt \omega \rho) \phi^{\text{Lz}}_{m_\ell}(\varphi) \label{eq:fockdarwin} \\
  \phi^{\text{FDr}}_{n, m}(\varrho) &\equiv \sqrt{\frac{2 \cdot n!}{(n + m)!}} \mathrm{e}^{-\varrho^2 / 2} \varrho^m L_n^{(m)}(\varrho^2) \notag \\
  \phi^{\text{Lz}}_{m_\ell}(\varphi) &\equiv \frac{1}{\sqrt{2 \pi}} \mathrm{e}^{\mathrm{i} m_\ell \varphi} \notag
\end{align}
and $L_n^{(m)}$ denotes the generalized Laguerre polynomial of degree $n$
with parameter $\mu$,
\begin{align*}
  L_n^{(m)}(u) = \frac{1}{n!} u^{-m} \mathrm{e}^u \frac{\mathrm{d}^n}{\mathrm{d} u^n} (\mathrm{e}^{-u} u^{m + n})
\end{align*}
The states are labeled by two quantum numbers: the principal quantum number $n \in \{0, 1, \ldots\}$ and the orbital angular momentum projection $m_\ell \in \mathbb Z$.

Additionally, since electrons are spin-$1/2$ fermions, they can occupy either of the two spin states $\chi_{-1/2}$ or $\chi_{+1/2}$.  For an electron in isolation, the choice is immaterial, but in a multi-electron system, this becomes crucial.  Thus for a many-body problem a single-particle basis state must contain both the spatial component \eqref{eq:fockdarwin} and the spin component,
...
\end{align}
where we have introduced a third quantum number: the spin projection $m_{\text s} \in \bigl\{-1/2, +1/2\bigr\}$.
...
\end{figure}
The eigenvalues of the single-particle Hamiltonian $\hat{h}$ are
given by
\begin{align*}
  \varepsilon_{n, m_\ell, m_{\text s}} \equiv (2 n + |m_\ell| + 1) \omega
\end{align*}
which satisfies
$\hat{h} \ket{\phi_{n, m_\ell, m_{\text s}}} = \varepsilon_{n,
...
our Hamiltonian $\hat{h}$ does not distinguish between them.  From
...
The many-body Hamiltonian $\hat{H}^{[1]}$ is the sum of the single-particle
Hamiltonians $\hat{h}_p$ over all particles $p$, where $p$ may now be
...
many-body Hamiltonian $\hat{H}^{[1]}$ in the form of a Slater determinant
...
\begin{align*}
  E^{[1]}_{p_1, \ldots, p_N} = \varepsilon_{p_1} + \cdots + \varepsilon_{p_N},
\end{align*}
which satisfies $\hat{H}^{[1]} \ket{\varnothing_{p_1, \ldots, p_N}} = E^{[1]}_{p_1, \ldots, p_N} \ket{\varnothing_{p_1, \ldots, p_N}}$.  For ground states, we may have degeneracies due to the shell structure of the single-particle basis.  However, if $N = k_{\text F} (k_{\text F} + 1)$ for some nonnegative integer $k_{\text F}$, there are just enough particles to form a closed-shell system.  In this case, there is a unique, well-isolated ground state.  The values of $N$ for which this occurs are typically referred to as \textit{magic numbers} and we call $k_{\text F}$ the \textit{number of filled shells} (or ``Fermi level'').  This completes the solution of the noninteracting problem.
...
Hamiltonian $H^{[1]}$ defined by \eqref{eq:onetwobodyhamiltonian} can be
rewritten in second quantization as
\begin{align} \label{eq:second_quantized_one_body_hamiltonian}
  \hat H^{[1]} = \sum_{p, q} H_{p, q}^{[1]} \hat a_p^\dagger \hat a_q^{}
\end{align}
where the \textit{matrix element}
$H_{p, q}^{[1]} \equiv \bra{\varnothing_p} \hat{H}^{[1]} \ket{\varnothing_q}$.  This
can likewise be extended to $\hat{H}^{[2]}$,
\begin{align} \label{eq:second_quantized_two_body_hamiltonian}
  \hat{H}^{[2]} = \frac{1}{4} \sum_{p, q, r, s} V_{p, q, r, s}^{}
  \hat a_p^\dagger \hat a_q^\dagger \hat a_s^{} \hat a_r^{}
\end{align}
where $U_{p, q} \equiv \langle \varnothing_p | \hat U | \varnothing_q \rangle$ and
$V_{p, q, r, s} \equiv \langle \varnothing_{p, q} | \hat{H}^{[2]} | \varnothing_{r, s} \rangle$.
The complete Hamiltonian is given by their sum:
\begin{align}
  \hat{H} = \sum_{p, q} H^{[1]}_{p, q} \hat{a}_p^\dagger \hat{a}_q + \frac{1}{4} \sum_{p, q, r, s} V_{p, q, r, s} \hat{a}_p^\dagger \hat{a}_q^\dagger \hat{a}_r \hat{a}_s  \label{eq:vacuumhamiltonian}
\end{align}
...
\begin{align}
  E_{\Phi} &= \sum_{j} n_j H^{[1] \prime}_{j, j} + \sum_{j, k} n_j n_k V'_{j, k, j, k} \label{eq:hfenergy}
\end{align}
where the transformed operators are defined as
\begin{align}
  H^{[1] \prime}_{p, q} &\equiv \sum_{r, s} C_{r, p}^* H_{r, s}^{[1]} C_{s, q}^{} \label{eq:hftransform1} \\
