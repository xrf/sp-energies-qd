...
\begin{align}
  ...
  \hat{X}^{(N + 1)}_u &= \sum_a x^{(u, +)}_a  \hat{a}^\dagger_a + \frac{1}{2} \sum_{a b i} x^{(u, +)}_{a b i} \hat{a}^\dagger_a \hat{a}^\dagger_b \hat{a}_i^{} + \cdots,  \\
  \label{eq:gen_removed}
  \hat{X}^{(N - 1)}_u &= \sum_i x_i^{(u, -)} \hat{a}_i^{} + \frac{1}{2} \sum_{i j a} x^{(u, -)}_{i j a} \hat{a}^\dagger_i \hat{a}_j^{} \hat{a}_a^{}  + \cdots.
\end{align}
...
\begin{align*}
    \hat{L}^{(N + 1)}_u &= \sum_a l^{(u, +)}_a \hat{a}_a^{} + \frac{1}{2} \sum_{i a b} l^{(u, +)}_{a b i} \hat{a}^\dagger_i \hat{a}_b^{} \hat{a}_a^{} + \cdots, \\
    \hat{L}^{(N - 1)}_u &= \sum_i l_i^{(u, -)} \hat{a}^\dagger_i + \frac{1}{2} \sum_{i j a} l^{(u, -)}_{i j a} \hat{a}^\dagger_i \hat{a}^\dagger_j \hat{a}_a^{} + \cdots
\end{align*}
These left-eigenvectors satisfy the left-eigenvalue problem, with left-eigenvalues $\varepsilon^{(\pm)}_u$, analogous to Eq.\ \eqref{eq:EOMIMSRG},
\begin{align*}
