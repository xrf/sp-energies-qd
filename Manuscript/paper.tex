...
where
\begin{align*}
  H^{[0]} &\equiv \sum_i n_i \langle i | \hat{H}^{[1]} | i \rangle + \frac{1}{2} \sum_{i j} n_i n_j \langle i j | \hat{H}^{[2]} | i j \rangle, \\
  H^{[1]}_{p q} &\equiv \langle p | \hat{H}^{[1]} | q \rangle + \sum_i n_i \langle p i | \hat{H}^{[2]} | q i \rangle, \\
...
\begin{align} \label{eq:cc_amps}
  \hat{T}_k \equiv \left(\frac{1}{k!}\right)^2 \sum_{\substack{a_1 \ldots a_k \\ i_1 \ldots i_k}} \bar{n}_{a_1} \ldots \bar{n}_{a_k} n_{i_1} \ldots n_{i_k} t^{[k]}_{a_1 \ldots a_k i_1 \ldots i_k} \normord{\hat{a}_{a_1}^\dagger \ldots \hat{a}_{a_k}^\dagger \hat a_{i_k}^{} \ldots \hat{a}_{i_1}^{}},
\end{align}
