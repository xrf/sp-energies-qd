...
Even though the model system contains 5 parameters $(N, m, \omega, e, \epsilon)$, many of them are redundant.  With an appropriate choice of units, the number of parameters in the system can be reduced to just two.  For rest of this paper, we shall use atomic units\footnote{Here, ``atomic unit'' is used in an \emph{abstract} sense: we do not assume (nor care) whether $m$, $e$, $\epsilon$ correspond to true values in vacuum, or some effective value in medium.} where $\hbar$, $m$, $e$ and $4 \pi \epsilon$ all have a value of one.  This implies that the units of energy are hartrees $E_{\mathrm{h}}$ and the units of length are Bohr radii $a$:
\begin{align*}
  E_{\mathrm{h}} &\equiv m \left(\frac{e^2}{4 \pi \epsilon \hbar}\right)^2 &
  a &\equiv \frac{4 \pi \epsilon \hbar^2}{m e^2}
\end{align*}
This leaves us with $(N, \omega)$ as the only necessary parameters to specify the quantum dot system, with $\omega$ in units of $E_{\mathrm{h}} / \hbar$.

The behavior of each electron in isolation is described by its kinetic energy $t(\bm{r})$ and the potential energy due to the trap $u(\bm{r})$, which combine to form the standard harmonic oscillator Hamiltonian $h(\bm{p}, \bm{r})$:
\begin{align*}
...
\end{align}
The operators $\hat{\bm p}_j$ and $\hat{\bm r}_j$ respectively denote momentum and position of the $j$-th particle.\footnote{The order in which particles are labeled is unimportant; it is used only for bookkeeping.}

The first step towards a solution is to solve \emph{just} the noninteracting part $\hat{H}^{[1]}$.  In this case, the many-body Hamiltonian reduces to $N$ independent single-particle Hamiltonians $\hat{h}$, each of the form:
\begin{align*}
  \hat{h} = \frac{1}{2} \hat{\bm{\nabla}}^2 + \frac{1}{2} \omega^2 \hat{\bm{r}}^2
\end{align*}
This is a 2-dimensional harmonic oscillator problem for a single particle with known analytic solutions.  There are various ways to construct the set of eigenfunctions: we shall use the Fock-Darwin states, which conserve the angular momentum $\hat{L}_{\mathrm{z}}$. In polar coordinates, they are given by\citep[Appx.\ A]{lohne2010coupled}
\begin{gather*}
