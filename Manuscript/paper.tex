e\documentclass[amsmath, amssymb, aps, floatfix, nofootinbib, preprintnumbers,showpacs, superscriptaddress, twocolumn]{revtex4-1}
\usepackage{bm}
...
Particle attached and particle removed equations of motion (EOM) methods can be coupled with either IM-SRG
or coupled cluster calculations. The principle idea is that one may construct a ladder operator $X$, 
which promotes the $A$-body ground state to any state in the $A$+$1$ or $A$-$1$ spectrum,
\begin{equation}\label{eq:ladder_def} 
  | \Psi^{(A \pm 1)}_\nu \rangle  = X^{(A \pm 1)}_\nu | \Psi^{(A)}_0 \rangle, 
\end{equation} 
where $X$ is in principle a linear combination of excitation and de-excitation operators which change particle number by one,
\begin{align}
  \label{eq:gen_attached}
  X^{(A+1)}_\nu &= \sum_p \bar{n}_p x^p  a^\dagger_p + \frac{1}{2} \sum_{pqr} \bar{n}_p \bar{n}_q n_r x^{pq}_r  a^\dagger_p a^\dagger_q a_r + \cdots,  \\
  \label{eq:gen_removed}
  X^{(A-1)}_\nu &= \sum_p n_p x_p a_p + \frac{1}{2} \sum_{pqr} n_p n_q \bar{n}_r  x_{pq}^r  a^\dagger_r a_q a_p  + \cdots. 
\end{align}
Substitution of equation~\ref{eq:ladder_def} into the energy eigenvalue problem $H | \Psi_\nu^{(A \pm 1)} \rangle = E^{(A\pm1)}_\nu | \Psi_\nu^{(A \pm 1)} \rangle$ gives
\begin{equation}\label{eq:EOM}
  [H,X^{(A \pm 1)}_\nu] | \Psi^{(A)}_0 \rangle = \omega_\pm X^{(A \pm 1)}_\nu | \Psi^{(A)}_0 \rangle,
\end{equation}
which constitutes a generalized eigenvalue problem for the amplitudes $x$, where $\omega_\pm$ are the
single particle addition and removal energies. The quality of this calculation depends on the ansatz for the $A$-body ground state, as well as the systematically
improvable truncation on the ladder operators. In this work we include 1p and 2p1h excitations in the $A$+1 ladder operator and likewise 1h and 2h1p operators for
the $A$-1 ladder operators. 

\subsubsection{IM-SRG}
After a single-reference ground state IM-SRG calculation, the Hamiltonian has been rotated such that the reference state is an eigenfunction with
corresponding eigenvalue $E^{(A)}_0$, which is the correlated $A$-body ground state energy. The EOM equation is therefore 
\begin{equation}\label{eq:EOMIMSRG} 
  [\bar{H},\bar{X}^{(A \pm 1)}_\nu] | \Phi^{(A)}_0 \rangle = \omega_\pm \bar{X}^{(A \pm 1)}_\nu | \Phi^{(A)}_0 \rangle,
\end{equation}
...
to be described appropriately by EOM-IMSRG if their wavefunctions are dominated by 1p (1h) contributions in the rotated frame. We use partial norms of the EOM ladder operators
