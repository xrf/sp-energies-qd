...
The second-quantized operators in \eqref{eq:second_quantized_hamiltonian} are constructed with respect to the vacuum state in the sense that their operator strings $\hat{a}_p^\dagger \hat{a}_q$ and $\hat{a}_p^\dagger \hat{a}_q^\dagger \hat{a}_s \hat{a}_r$ both have a vacuum expectation value of zero:
\begin{align*}
...
Before we discuss the results, let us briefly outline the concrete methodology used by our numerical experiments.  There is a some degree of flexibility in which many-body methods can be applied, which is not evident from reading the description of each method alone.
...
\begin{table}
  \centering
  \caption{Addition energy of a selection of quantum dot systems.  See Table \ref{tab:ground} for details.}
  \label{tab:add}
  \input{tab-add}
\end{table}
...
\begin{table}
  \centering
  \caption{Removal energy of a selection of quantum dot systems.  See Table \ref{tab:add} for details.}
  \label{tab:rm}
  \input{tab-rm}
\end{table}
...
\begin{table}
  \centering
  % The siunitx package somehow makes the columns unnecessarily wide so we have to compensate by shrinking the text size :/
  \footnotesize
  \caption{Extrapolated ground state energies for quantum dots with fit uncertainties.  The uncertainties are computed from the approximate Hessian used in Levenberg-Marquardt fitting procedure.  Extrapolations are done using 5-point fits where the number of shells $K$ ranges between $K_{\text{stop}} - 4$ and $K_{\text{stop}}$ (inclusive).}
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\linewidth]{fig-rel-slopes}
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{fig-by-num-shells-6-2-ground-0-normal}
  \caption{Ground state energy for quantum dots with 6 electrons plotted against $K$, the number of shells in the basis.  [[TODO: Results for FCI are from who?]] [[TODO: Add DMC results]]}
...
All calculations in this paper are performed with a relative numerical precision of about $10^{-5}$ or lower (sometimes \emph{much} lower).  This does not necessarily mean the results are as precise as $10^{-5}$, since numerical errors tend to accumulate at every step of the calculation, thus the precision of the final results may be an order of magnitude larger.
...
The discrepancy between IM-SRG results and the exact result is due to a combination of two factors: inadequate basis set, and operator truncation.
...
In Figure \ref{fig:gs} we display a selection of ground state energies calculated using the Hartree-Fock method, second-order (M\o ller-Plesset) perturbation theory (MPPT), and IM-SRG.  Results from diffusion Monte Carlo (DMC) \cite{hoegberget2013thesis} are shown as a dashed line for comparison.
