...
$\Psi^{\text{HF}}$ is a single Slater determinant constructed from an unknown
...
states $\{\psi_u\}$ has as many elements as the set of $\{\phi_pp\}$ and
...
in the Slater determinant $\Psi^{\text{HF}}$ and the remaining
...
\begin{align*}
  E^{\text{HF}} = \sum_{\underline u} C_{p, \underline u}^* H_{p, q}^{\text o} C_{q, \underline u}^{}
  + \sum_{\underline u, \underline v}
  C_{p, \underline u}^* C_{q, \underline v}^* V_{p, q, r, s}^{} C_{r, \underline u}^{} C_{s, \underline v}^{}
\end{align*}
...
\begin{align} \label{eq:fock}
  F_{p, q} \equiv H_{p, q}^{\text o} + \sum_{\underline u r s}
  C_{r, \underline u}^* V_{p, r, q, s}^{} C_{s, \underline u}^{}
\end{align}
and $\bm \epsilon$ is a vector of Lagrange multipliers with each element
$\epsilon_u$ associated with a particular state $u$.  [[move this sentence
...
$N_{\text F}$ occupied states.  One can define a set of \textit{quasiparticle
...
defined as:
\begin{align*}
  \hat b_p \equiv \hat a_{\overline p} + \hat a_{\underline p}^\dagger = \begin{cases}
    \hat a_{\overline p} & \text{if $\overline p$ is unoccupied in $\Phi$} \\
    \hat a_{\underline p}^\dagger & \text{if $\underline p$ is occupied in $\Phi$}
  \end{cases}
\end{align*}
Here we use an underline to indicate that only an occupied state is accepted
and an overline to indicate only an unoccupied state is accepted.  If the
state is not acceptable, then that term does not contribute to the overall
expression.

We can then define normal ordering with respect to the reference state $\Phi$:
namely, rearrange an operator string such that every creation operator appears
before an annihilation operator by repeatedly exchanging neighboring
operators, with each exchange costing a minus sign.  In this form, it is
guaranteed that the expectation value of the operator string with respect to
$\Phi$ is zero:
\begin{align*}
...
\begin{align*}
  ...
  -\hat{a}_{\underline q} \hat{a}_{\underline p}^\dagger
  + \hat{a}_{\underline p}^\dagger \hat{a}_{\overline q}
  + \hat{a}_{\overline p}^\dagger \hat{a}_{\underline q}
  + \hat{a}_{\overline p}^\dagger \hat{a}_{\overline q} =
  -\delta_{\underline q \underline p}
  + \hat{a}_p^\dagger \hat{a}_q
\end{align*}
This makes the normal ordering notation a useful and concise way of expressing
the right-hand side.

Using the normal ordering formalism, we can rewrite the Hamiltonian
$\hat H = \hat H^{\text o} + \hat V$ as
\begin{align*}
  \hat{H} = E + \sum_{p, q} F_{p, q} \normord{\hat{a}_p^\dagger \hat{a}_q} + \frac{1}{4} \sum_{p, q, r, s} V_{p, q, r, s} \normord{\hat{a}_p^\dagger \hat{a}_q^\dagger \hat{a}_r \hat{a}_s}
\end{align*}
where
\begin{align*}
  E &= \sum_{\underline p} H^{\text o}_{\underline p, \underline p} + \frac{1}{2} \sum_{\underline p, \underline q} V_{\underline p, \underline q, \underline p, \underline q} \\
  F_{p, q} &= H^{\text o}_{\underline p, \underline q} + \sum_{\underline r} V_{\underline p, \underline r, \underline q, \underline r} \\
\end{align*}
Notice that $E$ is in fact the Hartree-Fock energy and $F_{p, q}$ is the Fock matrix.
...
The commutator in the flow equations (\ref{eq:imsrgode}) guarantees that the
IM-SRG wave function $U^\dagger(s) \ket{\Phi}$ can be expanded in terms of
...
With this truncation, the generator
$\hat{\eta}$ can be written as
\[
...
are given by
% [[fix the equations to avoid requiring (anti)hermittivity in H]]
\begin{widetext}
\begin{align}
\frac{\D E}{\D s} &= \sum_{\underline p, \overline q} \left(\eta_{\underline p, \overline q} F_{a, \underline p} - \eta_{\overline q, \underline p} f_{\underline p, \overline q}\right) + \frac{1}{2} \sum_{\underline p, \underline q, \overline r, \overline s} \eta_{\underline p, \underline q, \overline r, \overline s} V_{\overline r, \overline s, \underline p, \underline q},
\label{eq:flow1}\\
\frac{\D F_{p, q}}{ds}&= \sum_r \left( \eta_{pr}^{(1)}f_{rq} +
...
\eta_{apij}^{(2)}v_{ijaq} + \frac{1}{2}\sum_{abi}\left( 1+\hat{P}_{pq}\right)
\eta_{ipab}^{(2)}v_{abiq},
\label{eq:flow2}
\end{align}
\begin{align}
\frac{\D V_{p, q, r, s}}{ds} &= \sum_t \left( 1-\hat{P}_{pq} \right) \left(
...
\begin{equation}
  ...
  = \left[ \hat{H}_{\text{off-diagonal}}, \hat{H} \right].
  \label{eq:etaWegner}
\end{equation}
